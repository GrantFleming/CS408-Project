...
\begin{code}
open import CoreLanguage using (Scope; Term; lib; const; Lib-Const; ess; thunk; Ess-Const; `; _∙_; bind)
open import Thinning using (_⊑_; _∘_; ι; _I; _⟨term_)
...
match (ess (` a)) (` c)  = just `
match (ess (` a)) _      = nothing

match (ess (s ∙ t)) (p ∙ q)  = do
  x ← match s p
  y ← match t q
  return (x ∙ y)
match (ess (s ∙ t)) _        = nothing

match (ess (bind t)) (bind p) = do
  x ← match t p
  return (bind x)
match (ess (bind x)) _        = nothing

match (thunk x) p = nothing
