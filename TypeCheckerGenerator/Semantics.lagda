...
$↞↞$ operater that we introduced in section \ref{section-corelanguage}.
...
elimination in order to access the required environments. The mechanics of
...
reduce {γ} rules checkprems ta ty el
  = do
    ...
    where open ElimRule
\end{code}
β-reduction failure depends solely on the existance of the rule, and the
...
to. We must, however, bear the rules in mind when we normalise and so a concious
...
to keep seperate the concerns of type-checking and normalisation and it is our
...
In order to satify the well-scopedness of the subterms, we must maintain a proof
