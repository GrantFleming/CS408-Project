module Test.SpecReadingTest where
  ...
  open import Data.List using (List; _∷_; []; foldr) renaming (map to lmap)
...
{-
  ...
  t : TypeRule
  subject t = ` "f" ∙ place ι ∙ ` "->" ∙ place ι
  premises t = (((` "⊤" ∙ place ι) ∙ place ι)) , (type (∙ (⋆ ∙)) ι ⇉ type (∙ ∙ ∙ ⋆) ι ⇉ ε ((` "f" ∙ ` "⊤" ∙ ` "->" ∙ ` "⊤") placeless))
  ...
  ans1 = construction (t ∷ t ∷ [] , r ∷ [] , []) {0} empty "lam x -> x"
  ans2 = construction (t ∷ t' ∷ [] , r ∷ [] , []) {0} empty "f alpha -> alpha"  
  ans = computation (t' ∷ t  ∷ [] , r' ∷ r ∷ [] , []) {0} empty "(lam x -> x : f alpha -> alpha) "
